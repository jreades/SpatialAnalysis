...
# Get the Datum
datum = ldn_lsoa.meta['crs']['datum']

# Set up a bounding box based on the size of the 
# LSOA Shapefile
bds    = ldn_lsoa.bounds
xmin, ymin = pyproj.transform(bng, wgs84, bds[0], bds[1])
xmax, ymax = pyproj.transform(bng, wgs84, bds[2], bds[3])

# Get a colormap for the scale
cm = plt.get_cmap('spectral')
num_colours = len(ldn_lsoa)
...
patches = []
for poly in ldn_lsoa: 
    #colour = cm(1. * len(filter(poly.contains, points)) / num_colours)
    colour = cm(1. * np.random.uniform(0,1))
    if poly['geometry']['type'] == 'Polygon':
        print("Polygon")
        p2 = Polygon(polyPatch(poly), fc=randomcolour(), ec='#cccccc', lw=0.2, alpha=1., zorder=1)
        patches.append(p2)
    elif poly['geometry']['type'] == 'MultiPolygon':
        print("Multi-Polygon")
        for p in poly: 
            pass
            #p3 = Polygon(polyPatch(p), fc=randomcolour(), ec='#ffffff', lw=0.2, alpha=1., zorder=1)
            #patches.append(p3)
...
#    if poly['geometry']['type'] == 'Polygon':
#        #p = polyPatch(poly)
#        #patches.append(Polygon(polyPatch(poly), fc=randomcolour(), ec='#999999', lw=0.2, alpha=1., zorder=1))
#        xy = poly['geometry']['coordinates'][0]
#        x, y = zip(*xy) 
#        if datum == 'OSGB36':
#            x1, y1 = pyproj.transform(bng,wgs84,x,y)
...
#            print e
#        plt.gca().add_patch(Polygon(xy, fc=randomcolour(), ec='#999999', lw=0.2, alpha=1., zorder=1))
#    elif poly['geometry']['type'] == 'MultiPolygon':
#        for p in poly: 
#            pass
#            #plot_coords(ax, polygon.exterior)
#            #patches.append(Polygon(polyPatch(p), fc=randomcolour(), ec='#999999', lw=0.2, alpha=1., zorder=1))
#
##pc = PatchCollection(patches, match_original=True)
##ax.add_collection(pc)
#ax.set_xticks([])
